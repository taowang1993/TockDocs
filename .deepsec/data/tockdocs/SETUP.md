# Agent setup for `tockdocs`

This is a deepsec scanning workspace. Project `tockdocs` was just registered
(target: `..`). Setup is incomplete — `data/tockdocs/INFO.md`
...
2. **Fill in `data/tockdocs/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id tockdocs
pnpm deepsec process --project-id tockdocs
```
